		...
		public static IEnumerable<Substring> Tokenize(this string str)
		{
			...
			while (i < length)
			{
				...
				if (str[i] == '"')
				{
					i++;
					int endPos2 = i;
					while (endPos2 < length && str[endPos2] != '"')
					{
						int num;
						if (str[endPos2] == '\\' && endPos2 < length - 1)
						{
							num = endPos2 + 1;
							endPos2 = num;
						}
					...
					yield return new Substring(str, i, endPos2 - i);
					i = endPos2 + 1;
				}
				else
				{
					int endPos2 = i;
					while (endPos2 < length && !char.IsWhiteSpace(str[endPos2]))
					{
						int num = endPos2 + 1;
						endPos2 = num;
					}
