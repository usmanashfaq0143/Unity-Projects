...
namespace UnityEngine.InputSystem.Utilities
{
	internal static class ArrayHelpers
	{
		...
		public static bool ContainsReference<TValue>(TValue[] array, TValue value) where TValue : class
		{
			if (array == null)
			{
				return false;
		...
		public static bool ContainsReference<TValue>(TValue[] array, int count, TValue value) where TValue : class
		{
		...
		public static int IndexOfReference<TValue>(this TValue[] array, TValue value, int count = -1) where TValue : class
		{
			return array.IndexOfReference(value, 0, count);
		}

		public static int IndexOfReference<TValue>(this TValue[] array, TValue value, int startIndex, int count) where TValue : class
		{
			if (array == null)
			{
				return -1;
		...
		public static void SwapSlice<TValue>(TValue[] array, int sourceIndex, int destinationIndex, int count)
		{
			...
			else
			{
				for (int j = 0; j < count; j++)
				{
					Swap(ref array[sourceIndex + j], ref array[destinationIndex + j]);
				}
			}
		}
		...
		public static void MoveSlice<TValue>(TValue[] array, int sourceIndex, int destinationIndex, int count)
		{
			if (count > 0 && sourceIndex != destinationIndex)
			{
				...
				while (destinationIndex != sourceIndex)
				{
					...
					destinationIndex = ((destinationIndex - sourceIndex < count * 2) ? (destinationIndex - 1) : (destinationIndex - count));
				}
			}
		}
