	...
	public sealed class InputEventTrace : IDisposable, IEnumerable<InputEventPtr>, IEnumerable
	{
		...
		public class ReplayController : IDisposable
		{
			...
			private void OnBeginFrame()
			{
				...
				{
					if (!MoveNext(skipFrameEvents: false, out eventPtr))
					{
						Finished();
						return;
					}
		...
		[SerializeField]
		private InlinedArray<Action<InputEventPtr>> m_EventListeners;
		...
		public unsafe bool Resize(long newBufferSize)
		{
			if (newBufferSize <= 0)
			{
		...
		private unsafe void OnInputEvent(InputEventPtr inputEvent, InputDevice device)
		{
			...
			for (int j = 0; j < m_EventListeners.length; j++)
			{
				m_EventListeners[j](new InputEventPtr((InputEvent*)eventBufferTail));
			}
		}
	}
}
