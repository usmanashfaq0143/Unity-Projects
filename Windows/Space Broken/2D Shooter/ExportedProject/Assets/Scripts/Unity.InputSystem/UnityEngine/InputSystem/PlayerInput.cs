...
namespace UnityEngine.InputSystem
{
	...
	public class PlayerInput : MonoBehaviour
	{
		...
		[NonSerialized]
		private InlinedArray<Action<PlayerInput>> m_DeviceLostCallbacks;

		[NonSerialized]
		private InlinedArray<Action<PlayerInput>> m_DeviceRegainedCallbacks;

		[NonSerialized]
		private InlinedArray<Action<PlayerInput>> m_ControlsChangedCallbacks;

		[NonSerialized]
		private InlinedArray<Action<InputAction.CallbackContext>> m_ActionTriggeredCallbacks;
		...
		public InputActionAsset actions
		{
			get
			{
				if (!m_ActionsInitialized && base.gameObject.activeSelf)
				{
					InitializeActions();
				}
				return m_Actions;
		...
		public bool neverAutoSwitchControlSchemes
		{
			...
			set
			{
				if (m_NeverAutoSwitchControlSchemes != value)
				{
					...
					if (base.enabled && m_OnUnpairedDeviceUsedHooked)
					{
		...
		public event Action<InputAction.CallbackContext> onActionTriggered
		{
			...
			remove
			{
				...
				int num = m_ActionTriggeredCallbacks.IndexOf(value);
				if (num != -1)
				{
					m_ActionTriggeredCallbacks.RemoveAtWithCapacity(num);
				}
			}
		}
		...
		public event Action<PlayerInput> onDeviceLost
		{
			...
			remove
			{
				...
				int num = m_DeviceLostCallbacks.IndexOf(value);
				if (num != -1)
				{
					m_DeviceLostCallbacks.RemoveAtWithCapacity(num);
				}
			}
		}
		...
		public event Action<PlayerInput> onDeviceRegained
		{
			...
			remove
			{
				...
				int num = m_DeviceRegainedCallbacks.IndexOf(value);
				if (num != -1)
				{
					m_DeviceRegainedCallbacks.RemoveAtWithCapacity(num);
				}
			}
		}
		...
		public event Action<PlayerInput> onControlsChanged
		{
			...
			remove
			{
				...
				int num = m_ControlsChangedCallbacks.IndexOf(value);
				if (num != -1)
				{
					m_ControlsChangedCallbacks.RemoveAtWithCapacity(num);
				}
			}
		}
		...
		public bool SwitchCurrentControlScheme(params InputDevice[] devices)
		{
			...
			SwitchCurrentControlScheme(inputControlScheme.Value.name, devices);
			return true;
		}
			...
			case PlayerNotifications.InvokeUnityEvents:
			{
				...
				foreach (ActionEvent actionEvent in array)
				{
					...
					if (!string.IsNullOrEmpty(actionId))
					{
						...
						else if (!string.IsNullOrEmpty(actionEvent.actionName))
						{
							Debug.LogError($"Cannot find action '{actionEvent.actionName}' with ID '{actionEvent.actionId}' in '{m_Actions}", this);
						}
						else
						{
							Debug.LogError($"Cannot find action with ID '{actionEvent.actionId}' in '{m_Actions}", this);
						}
					}
				}
				break;
		...
		private void OnDisable()
		{
			...
			if (num != -1)
			{
				ArrayHelpers.EraseAtWithCapacity(s_AllActivePlayers, ref s_AllActivePlayersCount, num);
			}
