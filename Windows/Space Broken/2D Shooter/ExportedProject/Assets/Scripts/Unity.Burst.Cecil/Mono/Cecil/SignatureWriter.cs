...
namespace Mono.Cecil
{
	internal sealed class SignatureWriter : ByteBuffer
	{
		...
		public void WriteMethodSignature(IMethodSignature method)
		{
			...
			int num = ((method is IGenericParameterProvider genericParameterProvider && genericParameterProvider.HasGenericParameters) ? genericParameterProvider.GenericParameters.Count : 0);
			if (num > 0)
			{
				b = (byte)(b | 0x10u);
			}
		...
		{
			if (value is TypeDefinition declaringType)
			{
				...
				{
					WindowsRuntimeProjections.Project(declaringType);
					WriteTypeReference(value);
					WindowsRuntimeProjections.RemoveProjection(declaringType);
					return;
				}
			}
			WriteTypeReference(value);
