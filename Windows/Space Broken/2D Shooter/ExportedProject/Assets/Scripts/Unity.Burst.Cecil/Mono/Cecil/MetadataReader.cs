	...
	internal sealed class MetadataReader : ByteBuffer
	{
		...
		private void InitializeDocuments()
		{
			if (metadata.Documents == null)
			{
				...
				for (uint num2 = 1u; num2 <= num; num2++)
				{
					...
					{
						HashAlgorithm = guid.ToHashAlgorithm(),
						Hash = hash,
						Language = guid2.ToLanguage(),
		...
		private ScopeDebugInformation ReadLocalScope(Row<uint, Range, Range, uint, uint, uint> record)
		{
			...
			if (record.Col2.Length != 0)
			{
				...
				{
					scopeDebugInformation.variables.Add(ReadLocalVariable(record.Col2.Start + num));
				}
			}
			if (record.Col3.Length != 0)
			{
				...
				{
					scopeDebugInformation.constants.Add(ReadLocalConstant(record.Col3.Start + num2));
				}
			}
			return scopeDebugInformation;
		}
		...
		private ConstantDebugInformation ReadLocalConstant(uint rid)
		{
			...
			{
				if (signatureReader.buffer[signatureReader.position] != byte.MaxValue)
				{
			...
			{
				value = (typeReference.IsTypeOf("System", "DateTime") ? ((object)new DateTime(signatureReader.ReadInt64())) : ((typeReference.etype != ElementType.Object && typeReference.etype != 0 && typeReference.etype != ElementType.Class) ? signatureReader.ReadConstantSignature(typeReference.etype) : null));
			}
			else
			{
		...
		public Collection<CustomDebugInformation> GetCustomDebugInformation(ICustomDebugInformationProvider provider)
		{
			...
			for (int i = 0; i < value.Length; i++)
			{
				if (value[i].Col1 == StateMachineScopeDebugInformation.KindIdentifier)
				{
					...
					collection.Add(new StateMachineScopeDebugInformation(signatureReader.ReadInt32(), signatureReader.ReadInt32()));
				}
				else if (value[i].Col1 == AsyncMethodBodyDebugInformation.KindIdentifier)
				{
					...
					Collection<InstructionOffset> collection2 = new Collection<InstructionOffset>();
					Collection<InstructionOffset> collection3 = new Collection<InstructionOffset>();
					uint rid = 0u;
					while (signatureReader2.CanReadMore())
					{
						collection2.Add(new InstructionOffset(signatureReader2.ReadInt32()));
						collection3.Add(new InstructionOffset(signatureReader2.ReadInt32()));
						rid = signatureReader2.ReadCompressedUInt32();
					}
					AsyncMethodBodyDebugInformation asyncMethodBodyDebugInformation = new AsyncMethodBodyDebugInformation(catchHandler);
					asyncMethodBodyDebugInformation.yields = collection2;
					asyncMethodBodyDebugInformation.resumes = collection3;
					asyncMethodBodyDebugInformation.move_next = GetMethodDefinition(rid);
				...
				else if (value[i].Col1 == EmbeddedSourceDebugInformation.KindIdentifier)
				{
					...
					int num = signatureReader3.ReadInt32();
					uint num2 = signatureReader3.sig_length - 4;
					CustomDebugInformation item = null;
					if (num == 0)
					{
						item = new EmbeddedSourceDebugInformation(signatureReader3.ReadBytes((int)num2), compress: false);
					}
					else if (num > 0)
					{
						MemoryStream stream = new MemoryStream(signatureReader3.ReadBytes((int)num2));
