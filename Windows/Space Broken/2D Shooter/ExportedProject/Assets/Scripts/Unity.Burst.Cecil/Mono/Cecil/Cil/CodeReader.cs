...
namespace Mono.Cecil.Cil
{
	internal sealed class CodeReader : BinaryStreamReader
	{
		...
		private void MoveBackTo(int position)
		{
			reader.context = null;
		...
		private void ReadStateMachineScope(StateMachineScopeDebugInformation state_machine_scope)
		{
			...
			Instruction instruction = GetInstruction(state_machine_scope.end.Offset);
			state_machine_scope.end = ((instruction == null) ? default(InstructionOffset) : new InstructionOffset(instruction));
		}
		...
		private object ReadOperand(Instruction instruction)
		{
			...
			{
				MetadataToken metadataToken = ReadToken();
				body.instructionTokens.Add(instruction, metadataToken);
				return GetString(metadataToken);
			}
