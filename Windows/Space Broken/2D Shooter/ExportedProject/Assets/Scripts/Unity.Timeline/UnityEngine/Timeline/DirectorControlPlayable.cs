...
namespace UnityEngine.Timeline
{
	public class DirectorControlPlayable : PlayableBehaviour
	{
		...
		{
			if (director.state != 0 && (!graph.IsPlaying() || (director.extrapolationMode == DirectorWrapMode.None && !(time < m_AssetDuration))) && director.state != 0 && ((director.extrapolationMode == DirectorWrapMode.None && time > m_AssetDuration) || !graph.IsPlaying()))
			{
				director.Pause();
			}
		}
