...
namespace UnityEngine
{
	...
	public class RuleTile : TileBase
	{
		...
		public virtual bool RuleMatches(TilingRule rule, Vector3Int position, ITilemap tilemap, ref Matrix4x4 transform)
		{
			...
			else if (rule.m_RuleTransform == TilingRuleOutput.Transform.MirrorY && RuleMatches(rule, position, tilemap, mirrorX: false, mirrorY: true))
			{
				transform = Matrix4x4.TRS(Vector3.zero, Quaternion.identity, new Vector3(1f, -1f, 1f));
				return true;
			}
		...
		public bool RuleMatches(TilingRule rule, Vector3Int position, ITilemap tilemap, int angle)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				Vector3Int rotatedPosition = GetRotatedPosition(rule.m_NeighborPositions[i], angle);
