		...
		public string ToString(string title)
		{
			...
			else
			{
				...
				foreach (UnityEngine.Object @object in array)
				{
					...
					{
						stringWriter.WriteLine($"{num}: {@object.GetType().FullName} [{@object.GetHashCode()}] \"{@object.name}\"");
					}
					else
					{
						stringWriter.WriteLine($"{num}: {@object.GetType().FullName} [{@object.GetHashCode()}]");
					}
					num++;
				}
			}
