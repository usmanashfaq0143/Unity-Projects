...
namespace Unity.VisualScripting
{
	public static class Serialization
	{
		...
		private static readonly HashSet<object> availableDependencies;
		...
		static Serialization()
		{
			...
			availableDependencies = new HashSet<object>();
		...
		{
			if (dependency is UnityEngine.Object @object)
			{
		...
		private static void CheckIfDependenciesMet(ISerializationDepender depender)
		{
			...
			{
				object item = WeakenSerializationDependencyReference(deserializationDependency);
				if (!availableDependencies.Contains(item))
				{
					flag = false;
					break;
				}
