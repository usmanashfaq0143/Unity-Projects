		...
		private bool OutputHasGraph(Flow flow)
		{
			...
			if (flow.GetValue(target, targetType) is GameObject gameObject)
			{
				if (gameObject != null)
				{
					...
					return components.Where((TMachine currentMachine) => (UnityEngine.Object)currentMachine != (UnityEngine.Object)null).Any((TMachine currentMachine) => currentMachine.graph != null && currentMachine.graph.Equals(macro.graph));
				}
			}
			else
			{
