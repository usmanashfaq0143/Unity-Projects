...
namespace UnityEngine.U2D.IK
{
	...
	public class IKChain2D
	{
		...
		public int transformCount
		{
			...
			set
			{
				m_TransformCount = Mathf.Max(0, value);
			}
		}
		...
		private bool Validate()
		{
			...
			if ((bool)target && IKUtility.IsDescendentOf(target, rootTransform))
			{
				return false;
			}
			return true;
		...
		public void Initialize()
		{
			if (!(effector == null) && transformCount != 0 && IKUtility.GetAncestorCount(effector) >= transformCount - 1)
			{
				...
				Transform parent = effector;
				int num = transformCount - 1;
				while ((bool)parent && num >= 0)
				{
					m_Transforms[num] = parent;
					m_DefaultLocalRotations[num] = parent.localRotation;
		...
		private void PrepareLengths()
		{
			...
			while ((bool)parent && num >= 0)
			{
				if ((bool)parent.parent && num > 0)
				{
					m_Lengths[num - 1] = (parent.position - parent.parent.position).magnitude;
				}
				parent = parent.parent;
				num--;
			}
		...
		{
			int num = (targetRotationIsConstrained ? transformCount : (transformCount - 1));
			for (int i = 0; i < num; i++)
			{
